...
exports.createCourse = function (req, res) {
    ...
    if (req.body.name && req.body.shortName && req.user.type === "teacher") {
        var newCourse = new global.models.courseModel({
        ...
        newCourse.save(function (err, course) {
            if (err) {
                ...
                global.models.courseModel.findOne({ _id: course._id }).populate("teacher").exec(function (err, course) {
                    if (err) {
...
exports.deleteCourse = function (req, res) {
    ...
    if (req.body.courseId && req.user.type === "teacher") {
        ...
        global.models.courseModel.findOne({ _id: req.body.courseId }).populate("teacher").populate("students").exec(function (err, course) {
            if (err) {
                response.error = "Could not fetch information.";
                res.status(400).json(response);
            } else if (course) {
                if (course.teacher._id == req.user._id) {
                    ...
                    course.teacher.save();
                    global.models.courseModel.remove({ _id: req.body.courseId }, function (err) {
                        if (err) {
...
exports.joinCourse = function (req, res) {
    ...
    if (req.body.courseId && req.user.type === "student") {
        global.models.courseModel.findOne({ _id: req.body.courseId }).populate("teacher").exec(function (err, course) {
            if (err) {
                response.error = "Could not fetch information.";
                res.status(400).json(response);
            } else if (course) {
...
exports.leaveCourse = function (req, res) {
    var response = {};

    if (req.body.courseId && req.user.type === "student") {
        global.models.userModel.findOne({ username: req.user.username }).populate("courses").exec(function (err, user) {
            if (err) {
                response.error = "Could not fetch information.";
                res.status(400).json(response);
            } else if (user) {
...
exports.getAllCourses = function (req, res) {
    ...
    global.models.courseModel.find({}).populate("teacher").exec(function (err, courses) {
        if (err) {
            response.error = "Could not fetch information.";
            res.status(400).json(response);
        } else {
